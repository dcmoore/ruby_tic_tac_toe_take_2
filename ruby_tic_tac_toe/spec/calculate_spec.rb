...
describe Calculate do
  ...
  it "self.is_game_over?(Board, rules) - returns 1 if X wins, returns 2 if O wins, returns 5 if it is a DRAW, and returns false if none of that happens" do
    ...
    Calculate.is_game_over?(@board, @rules).should_not == false
    @board.reset
    
    setup_x_win_on_full_board
    Calculate.is_game_over?(@board, @rules).should == X
    @board.reset
    
    setup_o_win_in_three_moves
    Calculate.is_game_over?(@board, @rules).should == O
    @board.reset

    setup_draw
    Calculate.is_game_over?(@board, @rules).should == DRAW
    @board.reset
    
    Calculate.is_game_over?(@board, @rules).should == false
  end
  ...
  it "self.is_game_over?(Board, rows_cols_diags_blocks) - checking to see if winning by controlling a 2X2 block works" do
    ...
    Calculate.is_game_over?(@board, "rows_cols_diags_blocks").should == X
    @board.reset
  end
